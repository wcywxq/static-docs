...
> expries 是 HTTP/1 的产物，表示资源会在设置的时刻后过期，需要再次发起请求，因为 expries 受限于本地事件，如果修改了本地事件，可能会造成缓存失效。
...
> Last-Modified 表示本地文件最后修改日期，If-Modified-Since 会将 Last-Modified 的值发送给服务器，询问无夫妻在该日期后资源是否有更新，如果有更新则将新资源发送回，否则返回 304
